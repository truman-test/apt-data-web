      ...
      <div className="text-center">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mt-4 rounded-lg bg-gray-100 p-4 text-left dark:bg-gray-800">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Error Details</p>
            <p className="mt-1 text-sm text-red-600 dark:text-red-400">{error.message}</p>
            {error.digest && (
              <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">Digest: {error.digest}</p>
            )}
          </div>
        )}
